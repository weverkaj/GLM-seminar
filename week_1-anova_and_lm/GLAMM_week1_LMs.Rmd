...
#Optional suggested packages to install
...
ggplot(data = iris, aes(y = petal_length, x = sepal_length)) +
  ...
  theme_bw()+
  ggpubr::stat_regline_equation(label.x = 7, label.y = 4) #Add some text with our lm formula
```
...
Keyword **Linear**. We expect the relationship between the response variable and the covariate(s) to be linear. We can check this in scatterplots like those above, or by plotting our model's residuals vs fitted $Y$ values:  

```{r}
plot(iris_lm2, 1)
```

The red line would be horizontal at 0 if there were no pattern, but it looks there may be some nonlinearity in our model.  
...
In this case, we can see if adding the *species* varaible to our model helps explain the nonlinear pattern we see in petal_length:  
```{r}
...
**Note**: The lack of a pattern in model residuals against fitted values (as above) or any covariate supports the assumption that residuals are independently distributed and uncorrelated.  
...
*Note*: A **'U'** shaped Q-Q plot indicates that the data is right (positively) or left (negatively)-skewed. An **'S'** shape can indicate that the data is under or over-dispersed.  
...
vif(iris_lm2) #model without species varaible
...
We typically don't expect any single observations to have significant influence on the relationship between covariates and the response variable. We can use a plot of "Cook's Distance" to determine whether individual observations could bias the fit of the model:  
...
There may be one or two observations of large influence, but all points are within Cook's distance (red dashed lines - not shown) so they shouldn't be problematic in our model.  
...
A significant parameter estimate for an interaction term indicates that these two variables co-dependently influence the value of the response variable. Interpretations of this parameter estimate can be a little trickier, and may be easier with some graphical methods.  
<br>  
...
Analysis of Variance (ANOVA) is a statistical analysis used to test whether three or more different groups have different means. Like regression, we can also model the effects some explanatory variable has on a response variable, only the explanatory variable ise categorical, not numeric.  
...
```
The `anova()` function runs a one-way ANOVA on our linear model. The output provides the degrees of freedom, the sum of squares (SS) and the Mean Squares (MS) for the species factor and the residuals, as well as an F and p-value for species.  
A significant p-value indicates that one or more significant differences exist between group means.  
<br>  


When used on a linear model built for ANOVA, the `summary()` function provides an output table similar to those above with our linear regression model. The difference is that with an ANOVA LM, this output uses the intercept as the reference level (species 1) and tests other levels (species 2 & 3) against it. (It does not test the significance of the species factor overall.)   
```{r}
summary(iris_lm4)
...
*Note*: If you compare the differences between means in the TukeyHSD output and the group-level parameter estimates in the `summary()` output, you'll see that they are the same!  
...
The type of Sum of squares used can determine the accuracy and interpretability of your ANOVA when you have two or more covariates and interactions between covariates.  
...
- It is usually difficult to interpret main effects when interactions are present  
- These contrasts and SS types also apply to linear regression!  
<br>  

To run an ANOVA with Type II or III SS, you need to change the type of contrast in R. Instead of comparing factor levels to a reference level (as in treatment contrasts), sum-to-zero contrasts compare each level to the overall mean. This helps with interpreting the model.  
```{r}
...
Anova(iris_lm5a, type = 3)
Anova(iris_lm5a, type = 2)

```

In the case of a linear model with species and sep_width_f, the interaction term is not significant so a Type II ANOVA would be more appropriate than Type III.  
...
## ANCOVA  
An Analysis of Covariance (ANCOVA) copmares means of a response variable among 2+ groups, accounting for variability of a **numerical** variable(s). It can be defined as one-way (a single factor with 2+ groups) or two/multi-way (two or more factors).  
...
#see ?summlm and ?scale_mod for more scaling options
...
```{r eval = FALSE, echo = FALSE}
#Other build in datasets include:  
